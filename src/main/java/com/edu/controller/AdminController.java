...
/**
 * 이 클래스는 Admin관리자단을 접근하는 클래스 <- 디스패쳐 서블렛(게이트웨이) 기능을합니다
...
public class AdminController {
	...
	//jsp에서 게시판 생성관리에 접근 Get/Post 접근할때 URL을 bbs_type로 지정합니다
	...
	//bbs_type_insert.jsp의 입력폼에서 전송된 값을 BoardTypeVO 자동담겨서 {구현} 단 자동으로 값이 바인딩되려면 폼name과, VO 멤버변수명과 동일
	...
	public String insertBoardType(BoardTypeVO boardTypeVO) throws Exception { //입력처리1
		...
		//리다이렉트(뒤로가기 데이터 사라짐)는 절대경로 forward: 이동이 가능(뒤로가기 데이터가 살아있음)
	}
	//게시판 생성관리는 이 기능은 사용자단에서 UI를 사용할일이 없기때문에 Read Update를 1개로 사용
