...
/* PC용 메인페이지 스타일지정 1066px~무한대까지 재정의 */
...
$(document).ready(function() {
	...
	var slidePlayHide = setTimeout(function(){
		$('.rollplay').css('display','none');},3000); //3초후에 roll클래스 플레이버튼 영역을 숨김
	$('.rollstop a').click(function(){
		//this는 클리한 본인 태그를 말합니다
		$(this).parent().hide(); //현재 stop버튼 숨김
		$('.rollplay').css('display','inline-block');
		if(slideAuto) {
			clearTimeout(slideAuto); //slideAuto변수가 없다면, play함수를 실행중지
		}
	});
	...
	$('.rollingbtn li.seq a').each(function(index){ 
		$(this).click(function(){
			$('rollplay').hide();
			$('rollstop').css('display','inlineblock');
			if(slideAuto) {
